...
public class SpeedyToolOrb extends SpeedyToolSimple
{
   ...
   * @return   returns the list of blocks in the selection (may be zero length)
   */
  protected Pair<List<BlockPos>, EnumFacing> selectFillBlocks(MovingObjectPosition blockUnderCursorMOP,
...
//    MovingObjectPosition startBlock = BlockMultiSelector.selectStartingBlock(blockUnderCursorMOP, BlockMultiSelector.BlockTypeToSelect.SOLID_OK, player, partialTick);
    ...
    BlockPos blockUnderCursor = blockUnderCursorMOP.getBlockPos();

    boolean diagonalOK =  controlKeyIsDown;

    World world = player.worldObj;
    IBlockState iBlockState = world.getBlockState(blockUnderCursor);
    if (iBlockState.getBlock() == Blocks.air) {
      return new Pair<List<BlockPos>, EnumFacing>(new ArrayList<BlockPos>(), EnumFacing.UP);
    }

    BlockWithMetadata blockWithMetadata = new BlockWithMetadata(iBlockState);
    ...
    return new Pair<List<BlockPos>, EnumFacing> (selection, blockUnderCursorMOP.sideHit);
  }
